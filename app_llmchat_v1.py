...
def file_loader(file_extension, file_name):
    if file_extension == '.pdf':
        ...
        loader = UnstructuredImageLoader(file_name, mode="elements", strategy="fast")
    else:
        st.write("Document format is not supported.")
    return loader

# Re-build retriever
chromadb = Chroma(collection_name="uploaded_docs", embedding_function=embeddings,client_settings=Settings(anonymized_telemetry=False, is_persistent=False))
if data_name == "Uploaded Files":
    if uploaded_files:
        with st.spinner('Loading file...'):
            bytes_data = uploaded_files.read()
            file_name = os.path.join('./uploaded_docs/', uploaded_files.name)
            with open(file_name,'wb') as f:
                f.write(bytes_data)
            
            name, file_extension = os.path.splitext(file_name)
            loader = file_loader(file_extension, file_name)
...
if clear_button:
    ...
    try:
        Chroma.delete_collection(client_settings=Settings(anonymized_telemetry=False, is_persistent=False))
    except:
